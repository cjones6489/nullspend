...
export async function invalidateProxyCache(params: {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (res.ok) {
        log.info(
          { action: params.action, ownerId: params.ownerId,
      ...
      log.error(
        { status: res.status, action: params.action, ownerId: params.ownerId,
      ...
      log.error(
        { err, action: params.action, ownerId: params.ownerId,
          entityType: params.entityType, entityId: params.entityId, attempt },
