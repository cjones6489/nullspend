...
export default function MarginsPage() {
  ...
  const tierCounts = useMemo(() => {
    if (!data?.customers) return { healthy: 0, moderate: 0, at_risk: 0, critical: 0 };
    const counts = { healthy: 0, moderate: 0, at_risk: 0, critical: 0 };
    for (const c of data.customers) counts[c.healthTier]++;
    return counts;
  }, [data]);

  const sorted = useMemo(() => {
    if (!data?.customers) return [];
    const items = healthFilter
      ? data.customers.filter((c) => c.healthTier === healthFilter)
      : [...data.customers];
    items.sort((a, b) => {
        ...
        <div>
          ...
          <p className="mt-1 text-[13px] text-muted-foreground">
            {summary && !isDisconnected && !isEmpty
              ? `${data!.customers.length} customers | ${summary.blendedMarginPercent.toFixed(0)}% blended margin | ${summary.criticalCount + summary.atRiskCount} at risk`
      ...
      {data && !isEmpty && !isDisconnected && (
        <>
          ...
          <div className="flex flex-wrap gap-2">
            {(["healthy", "moderate", "at_risk", "critical"] as HealthTier[]).map((tier) => {
              const config = HEALTH_CONFIG[tier];
              const count = tierCounts[tier];
              const isActive = healthFilter === tier;
              return (
                <button
                  key={tier}
                  type="button"
                  onClick={() => setHealthFilter(isActive ? null : tier)}
                  className={cn(
